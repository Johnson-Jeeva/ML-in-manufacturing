# =============================================================================
# Minitab Python Integration — ANN Predictive Modelling Script (Single Model)
# Model  : Deep ANN with pyramid architecture + StandardScaler pipeline
#          Auto-tuned hyperparameters for maximum accuracy on any dataset size
# Output : Performance metrics, permutation feature importance, loss curve,
#          actual vs predicted, residual diagnostics, SHAP, PDP, ICE,
#          learning curves — all displayed in the Minitab output pane
#
# WHY ONE MODEL:
#   Running 3 ANNs (each up to 2000 epochs) + SHAP KernelExplainer × 3
#   is the primary cause of long runtimes. A single well-tuned deep ANN
#   with early stopping consistently matches or beats a 3-model average
#   at a fraction of the compute cost.
#
# SPEED OPTIMISATIONS vs previous script:
#   • 1 model instead of 3          → ~3× faster training
#   • SHAP background rows   50     → ~2× faster KernelExplainer
#   • SHAP max rows         150     → faster SHAP pass
#   • Perm importance reps    8     → faster, still stable
#   • Learning curve points   6     → fewer CV re-trains
#   • n_jobs = -1 everywhere        → full CPU parallelism
#
...
PERM_IMP_REPEATS     =  8    # permutation importance repeats (8 is fast + stable)
SHAP_MAX_ROWS        = 150   # validation rows sent to KernelExplainer
SHAP_BACKGROUND_ROWS =  50   # kmeans background points (50 is sufficient)
...
plots_dir  = os.path.join(script_dir, "Plots_ANN_Fast")
...
# -----------------------------------------------------------------------------
# 7. BUILD THE BEST ANN MODEL
#
#    Architecture  : Pyramid  wide → mid → narrow  (captures hierarchy of features)
#    Width         : auto-scales with n_features — width = clamp(n_features*4, 64, 512)
#    Activation    : relu — fastest convergence, best for regression
#    Solver        : adam — adaptive moment estimation, best all-round solver
#    Learning rate : adaptive — halves automatically when training stalls
#    alpha (L2)    : 1e-3 — balanced regularisation; prevents overfit without
#                    crippling capacity
#    Early stopping: True — monitors 10 % internal val split, stops when no
#                    improvement for 40 epochs → avoids wasted epochs entirely
#    max_iter      : 2000 — generous ceiling; early stopping fires well before this
#    tol           : 1e-5 — tight convergence threshold for precision
#    n_jobs        : -1 via Pipeline parallelism where possible
#
#    Wrapped in a StandardScaler Pipeline — critical for ANNs because unscaled
#    features cause gradient imbalance that severely harms convergence.
#    Scaler is fitted on TRAINING data only to prevent leakage.
# -----------------------------------------------------------------------------
...
if predict_mode:
    ...
    mtb.add_message(
        ...
        f"Model           : Deep ANN (pyramid architecture)\n"
        f"Target          : {target_col}"
    )
    sys.exit()
...
mtb.add_message(
    ...
    f"Model   : Deep ANN — pyramid ({int(np.clip(n_features*4,64,512))} "
    f"→ {max(32,int(np.clip(n_features*4,64,512))//2)} "
    f"→ {max(16,int(np.clip(n_features*4,64,512))//4)}) neurons\n"
    f"Scaler  : StandardScaler fitted on training split only"
)
...
n_iter     = getattr(mlp_step, "n_iter_", "N/A")

arch = mlp_step.hidden_layer_sizes
...
mtb.set_note(
    f"ANN Results  |  Target: {target_col}  |  "
    f"Predictors: {n_features}  |  Rows: {n_rows}  |  "
    f"Architecture: {arch_str}  |  Epochs: {n_iter}  |  Split: 70/30"
)
...
mtb.add_table(
    columns=[
        ["Train R-squared (%)", "Validation R-squared (%)",
         ...
         "L2 Regularisation (alpha)", "Initial Learning Rate",
         "Training Epochs (actual)"],
        [round(r2_train * 100, 4), round(r2_test * 100, 4),
         round(rmse_train, 4),     round(rmse_test, 4),
         round(mad, 4),
         float(mlp_step.alpha),
         float(mlp_step.learning_rate_init),
         float(n_iter) if n_iter != "N/A" else 0.0],
    ],
    headers=["Metric", "Value"],
    title="Deep ANN — Performance Metrics",
    footnote=(
        f"Target: {target_col}  |  Predictors: {n_features}  |  "
        f"Training rows: {len(y_train)}  |  Validation rows: {len(y_test)}  |  "
        f"StandardScaler applied (fitted on training set only)"
    )
)

# -----------------------------------------------------------------------------
# SECTION 2 — Model Configuration (text — separate table avoids type error)
# -----------------------------------------------------------------------------
mtb.add_table(
    columns=[
        ["Architecture (neurons per layer)", "Activation",
         "Solver", "Learning Rate Schedule",
         "Early Stopping", "Batch Size"],
        [arch_str,
         str(mlp_step.activation),
         str(mlp_step.solver),
         str(mlp_step.learning_rate),
         str(mlp_step.early_stopping),
         str(mlp_step.batch_size)],
    ],
    headers=["Hyperparameter", "Setting"],
    title="Deep ANN — Model Configuration",
    footnote="Early stopping monitors an internal 10 % validation split with patience=40 epochs."
)
...
try:
    ...
    mtb.add_table(
        ...
        title=f"Deep ANN — Permutation Feature Importance (Top {top_n} of {n_features})",
        footnote=(
...
try:
    if perm_mean is not None:
        ...
        colors     = plt.cm.Purples(np.linspace(0.35, 0.9, top_n_plot))
        ...
        ax.set_title(
            f"Deep ANN — Permutation Feature Importance\n"
            f"Top {top_n_plot} of {n_features} Predictors",
            fontweight="bold"
        )
        ax.axvline(0, color="black", linewidth=0.5)
        fig.tight_layout()
        save_and_push(fig, "ANN_feature_importance.png")
except Exception as e:
...
# -----------------------------------------------------------------------------
try:
    loss_curve = list(getattr(mlp_step, "loss_curve_", []))
    val_scores = list(getattr(mlp_step, "validation_scores_", []))
    ...
    if loss_curve:
        ...
        ax.set_title(
            f"Deep ANN — Training Loss Curve\n"
        ...
        fig.tight_layout()
        save_and_push(fig, "ANN_loss_curve.png")
except Exception as e:
...
for ax, y_true, y_pred, label, idx in [
    ...
    ax.scatter(y_pred[idx], y_true[idx],
               color="mediumpurple", s=max(4, 40 - len(idx)//50), alpha=0.55)
    ax.plot([mn, mx], [mn, mx], "k--", linewidth=1, label="Perfect fit")
    ax.set_xlabel("Predicted")
    ax.set_ylabel("Actual")
    ax.set_title(label)
    ax.legend(fontsize=8)
fig.suptitle(f"Deep ANN — Actual vs Predicted{note_sub}", fontweight="bold")
fig.tight_layout()
save_and_push(fig, "ANN_actual_vs_predicted.png")
...
axes[0].scatter(pred_sub, res_sub,
                color="mediumpurple", s=max(4, 40 - len(res_idx)//50), alpha=0.55)
...
axes[1].hist(residuals, bins=n_bins, color="mediumpurple", edgecolor="white", alpha=0.85)
axes[1].axvline(0, color="black", linewidth=1, linestyle="--")
axes[1].set_xlabel("Residual Value")
axes[1].set_ylabel("Frequency")
axes[1].set_title("Residual Distribution  (all validation rows)")

fig.suptitle("Deep ANN — Residual Analysis (Validation Set)", fontweight="bold")
fig.tight_layout()
save_and_push(fig, "ANN_residual_analysis.png")

# -----------------------------------------------------------------------------
# PLOTS 5–8 — SHAP  (KernelExplainer with tight sampling caps for speed)
# -----------------------------------------------------------------------------
try:
    ...
    plt.title(
        f"Deep ANN — SHAP Beeswarm{shap_note}\n"
        f"Top {shap_display} of {n_features} features",
        fontweight="bold", pad=12
    )
    fig = plt.gcf()
    fig.tight_layout()
    save_and_push(fig, "ANN_shap_beeswarm.png")
    ...
    plt.title(
        f"Deep ANN — SHAP Waterfall{shap_note}\n"
        f"Obs #{worst_idx+1}  "
    ...
    fig = plt.gcf()
    fig.tight_layout()
    save_and_push(fig, "ANN_shap_waterfall.png")
    ...
    ax.set_title(
        f"Deep ANN — SHAP Dependence\n"
        f"'{top_feat}'  coloured by  '{color_feat}'{shap_note}",
        fontweight="bold"
    )
    fig.tight_layout()
    save_and_push(fig, "ANN_shap_dependence.png")
    ...
    fig.tight_layout()
    save_and_push(fig, "ANN_shap_force_summary.png")
...
# -----------------------------------------------------------------------------
# PLOT 9 — Partial Dependence Plots (top features by permutation importance)
# -----------------------------------------------------------------------------
try:
    if perm_mean is not None:
    ...
    PartialDependenceDisplay.from_estimator(
        ...
        line_kw={"color": "mediumpurple", "linewidth": 2}
    )
    ...
    fig.suptitle(
        f"Deep ANN — Partial Dependence Plots (PDP)\n"
        f"Top {n_pdp} Features by Permutation Importance",
        fontweight="bold"
    )
    fig.tight_layout()
    save_and_push(fig, "ANN_pdp.png")
except Exception as e:
...
try:
    ...
    PartialDependenceDisplay.from_estimator(
        ...
        line_kw={"alpha": ice_alpha, "color": "mediumpurple"},
        pd_line_kw={"color": "red", "linewidth": 2.5, "label": "PDP mean"}
    )
    ax.set_title(
        f"Deep ANN — ICE Plot  (top feature: '{top1_feat}')\n"
        f"{ice_lines} individual lines  |  Red = PDP mean",
        fontweight="bold"
    )
    ax.legend(fontsize=8)
    fig.tight_layout()
    save_and_push(fig, "ANN_ice_plot.png")
except Exception as e:
...
try:
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(train_sizes, train_mean, "o-", color="mediumpurple", label="Training R²")
    ...
    fig.tight_layout()
    save_and_push(fig, "ANN_learning_curves.png")
except Exception as e:
...
mtb.add_table(
    ...
    headers=["Obs", "Actual", "ANN Predicted", "Absolute Error"],
    title="Deep ANN — Predictions (Validation Set)",
    footnote=(
        f"Validation set: {len(y_test)} observations (30 % holdout)  |  "
        f"Target: {target_col}  |  "
        f"Architecture: {arch_str}  |  "
        f"Epochs trained: {n_iter}"
    )
)