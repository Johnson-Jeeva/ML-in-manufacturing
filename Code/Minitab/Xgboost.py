...
# Required arguments
if len(sys.argv) < 3:
    mtb.add_message("Usage: pysc 'XGBoost_model.py' <Target_Col> <Max_Col> [--trees N] [--depth N] [--lr VALUE]")
    sys.exit(1)
...
for i, arg in enumerate(sys.argv):
    if arg == "--trees" and i + 1 < len(sys.argv):
        n_trees = int(sys.argv[i + 1])
...
# ---------------------------
mtb.add_message(f"Model: XGBoost Regressor")
mtb.add_message(f"Parameters: Trees={n_trees}, Depth={max_depth}, Learning Rate={learning_rate}")
...
plt.title("Training - Actual vs Predicted (XGBoost)")
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.tight_layout()
plt.savefig("XGB_train_actual_vs_predicted.png", dpi=300)
mtb.add_image("XGB_train_actual_vs_predicted.png")
...
plt.title("Test -  Actual vs Predicted (XGBoost)")
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.tight_layout()
plt.savefig("XGB_test_actual_vs_predicted.png", dpi=300)
mtb.add_image("XGB_test_actual_vs_predicted.png")

# ---------------------------
# Plot 3: Top 25 Feature Importances
# ---------------------------
plt.figure(figsize=(10, 8))
plot_importance(model, max_num_features=25, importance_type='gain', height=0.5)
plt.title("Top 25 Feature Importances (XGBoost)")
plt.tight_layout()
plt.savefig("XGB_feature_importance.png", dpi=300)
mtb.add_image("XGB_feature_importance.png")
...
plt.tight_layout()
plt.savefig("XGB_residual_plot.png", dpi=300)
mtb.add_image("XGB_residual_plot.png")