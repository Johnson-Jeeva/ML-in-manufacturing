...
# 1. Remove zero or near-zero variance columns
...
# ---------------------------
alphas = np.logspace(-2, 3, 80)  # Start from 0.01 for stability
model = LassoCV(alphas=alphas, cv=5, random_state=42, max_iter=20000, tol=1e-4, selection='random')
...
# ---------------------------
mtb.add_message(f"Model: LASSO Regression (alpha={model.alpha_:.6f})")
...
plt.ylabel("Actual")
plt.tight_layout()
plt.savefig("LASSO_train_actual_vs_predicted.png", dpi=300)
mtb.add_image("LASSO_train_actual_vs_predicted.png")
...
plt.ylabel("Actual")
plt.tight_layout()
plt.savefig("LASSO_test_actual_vs_predicted.png", dpi=300)
mtb.add_image("LASSO_test_actual_vs_predicted.png")
...
if len(nonzero_idx) > 0:
    ...
    plt.title("Top 25 LASSO Non-zero Coefficients")
    plt.xlabel("Coefficient Value")
    plt.tight_layout()
    plt.savefig("LASSO_top25_coefficients.png", dpi=300)
    mtb.add_image("LASSO_top25_coefficients.png")
else:
    mtb.add_message("No non-zero coefficients found with current alpha.")
...
plt.tight_layout()
plt.savefig("LASSO_residual_analysis.png", dpi=300)
mtb.add_image("LASSO_residual_analysis.png")